...
const superSoup = { // in a recipe, the values are the quantity of that ingredient
  ...
  function mostExpensiveIngredient(store){
    ...
    for(let key in store){
        if(store[key]>mostExpensive){
            mostExpensive = store[key];
            index = key;
        }
    }
    return index;
  ...
  function costOfRecipeInStore(recipe, store){
    ...
    let totalCost = 0;
    for(let key in recipe){
        totalCost += store[key]*recipe[key];
    }
    return totalCost;
  }
  ...
  function cheapestStoreForRecipe(recipe, storeCollection){
    ...
    let leastCost = 1000;
    let cheapStore = "";
    for(let store in storeCollection){
        if(costOfRecipeInStore(recipe, storeCollection[store])<leastCost){
            leastCost = costOfRecipeInStore(recipe, storeCollection[store]);
            cheapStore = store;
        }
    }
    return cheapStore;
  ...
  function writeShoppingList(recipeList) {
    ...
    // but containing the quantities for all recipes in the lis
    let newObject = {};

    for(let recipe of recipeList){
        for(let i in recipe){
            if(newObject.hasOwnProperty(i)){
                newObject[i] += recipe[i];
            } else {
                newObject[i] = recipe[i];
            }
        }
  ...
  function possibleRecipes(recipeList, store, sum) {
    ...
    let moneyLeft = sum;
    for(let recipe of recipeList){
        if(moneyLeft - costOfRecipeInStore(recipe,store)>=0){
